...
export function createRouter(config = {}) {
  ...
  // ─── Master-db mirror (bridge until shared Postgres lands) ───────────────
  // The worker on hetzner writes its local SQLite, then POSTs here to replicate
  // into the dashboard's SQLite. One-directional. Idempotent-where-possible.
  router.post('/admin/mirror/workflow-instances/create', authenticateAdmin, express.json(), (req, res) => {
    try {
      const {
        work_item_id, workflow_name, current_step,
        module_id, cycle_id, started_at, last_event_at, metadata
      } = req.body || {};
      if (!work_item_id || !workflow_name || !current_step) {
        ...
        const info = db.prepare(
          `INSERT INTO workflow_instances
           ...
           VALUES (?, ?, 1, ?, 'running', ?, ?, ?, ?, ?)`
              ...
              started_at ?? Date.now(), last_event_at ?? Date.now(),
              metadata ?? null);
        return res.json({ id: info.lastInsertRowid });
      } catch (e) {
        if (e.code === 'SQLITE_CONSTRAINT_UNIQUE') {
          return res.json({ id: null, skipped: 'already_active' });
        }
        throw e;
      }
    } catch (e) {
  ...
  router.post('/admin/mirror/workflow-instances/update', authenticateAdmin, express.json(), (req, res) => {
    try {
      const {
        work_item_id, workflow_name,
        revision, current_step, status, last_event_at,
      ...
      if (!current) {
        // The mirror arrived before the create — stash a minimal row so we
        // don't lose it. Status defaults to whatever the update carries.
        const info = db.prepare(
          `INSERT INTO workflow_instances
             (work_item_id, workflow_name, revision, current_step, status,
              started_at, last_event_at, last_job_id, metadata)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
        ).run(work_item_id, workflow_name, revision ?? 1, current_step ?? '?',
              status ?? 'running', last_event_at ?? Date.now(),
              last_event_at ?? Date.now(), last_job_id ?? null, metadata ?? null);
        return res.json({ id: info.lastInsertRowid, created: true });
      }
      db.prepare(
        `UPDATE workflow_instances
            SET revision=?, current_step=?, status=?, last_event_at=?,
                exhausted_at=?, last_job_id=?, metadata=?
          WHERE id=?`
      ).run(revision, current_step, status, last_event_at,
            exhausted_at ?? null, last_job_id ?? null, metadata ?? null,
            current.id);
  ...
  router.post('/admin/mirror/job-events', authenticateAdmin, express.json(), (req, res) => {
    try {
      const { job_id, seq, event_type, event_subtype, payload } = req.body || {};
      if (!job_id || typeof seq !== 'number' || !event_type) {
        return res.status(400).json({ error: 'missing fields' });
      }
      const db = getMasterDatabase();
      try {
        db.prepare(
          `INSERT INTO agent_job_events (job_id, seq, event_type, event_subtype, payload)
           VALUES (?, ?, ?, ?, ?)`
        ).run(job_id, seq, event_type, event_subtype ?? null,
  ...
  router.post('/admin/mirror/job-log', authenticateAdmin, express.json(), (req, res) => {
    try {
      const { job_id, agent, step, status, error, duration_ms } = req.body || {};
      if (!job_id || !agent || !step || !status) {
        return res.status(400).json({ error: 'missing fields' });
      }
      const db = getMasterDatabase();
      db.prepare(
        `INSERT INTO agent_job_log (job_id, agent, step, status, error, duration_ms)
         VALUES (?, ?, ?, ?, ?, ?)`
      ).run(job_id, agent, step, status, error ?? null, duration_ms ?? null);
      res.json({ ok: true });
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  ...
  router.post('/admin/mirror/memory-write', authenticateAdmin, express.json(), (req, res) => {
    try {
      const { job_id, memory_id } = req.body || {};
      if (!job_id || !memory_id) return res.status(400).json({ error: 'missing fields' });
      const db = getMasterDatabase();
      db.prepare(
