...
export async function mountMcpHttp(app, { server, token, path = '/mcp' } = {}) {
  ...
  // GET — OpenCode's MCP SDK does a GET first expecting SSE (streamable HTTP).
  // The MCP SDK transport expects POST-first initialization, so we can't
  // route GET through it. Return a minimal SSE stream that closes immediately.
  app.get(path, (req, res) => {
    ...
    // Return JSON content-type — the SDK expects to parse this as JSON,
    // not SSE text. This satisfies the GET health check without opening
