...
async function happy() {
  ...
  expectEqual((await loadInstance({ work_item_id: wi, workflow_name: 'work-item' })).status, 'done', 'instance done');
}

async function replan() {
  const wi = `wi-replan-${Date.now()}`;
  await createInstance({ work_item_id: wi, workflow_name: 'work-item', current_step: 'qa' });
  ...
  expectEqual((await loadInstance({ work_item_id: wi, workflow_name: 'work-item' })).status, 'awaiting_approval', 'parent awaits');
  ...
  expectEqual((await loadInstance({ work_item_id: wi, workflow_name: 'work-item' })).revision, 2, 'parent rev=2');
  expectEqual((await loadInstance({ work_item_id: wi, workflow_name: 'work-item' })).status, 'running', 'parent running again');
}
