...
#
# devpanl override of /code/plane/settings/common.py.
# Mounted into plane-api (and plane-migrator) via docker-compose. Only delta
# vs upstream `makeplane/plane-backend:stable` is the DRF throttle rate:
# upstream caps anon at 30/minute, which throttles our 5-dev × N-agent
# workload within seconds of a bootstrap. We bump anon to 10000/minute so the
# throttle stays as a circuit-breaker against runaway loops only. asset_id
# kept at upstream default. To upgrade Plane, re-pull the upstream file with
#   docker exec plane-api cat /code/plane/settings/common.py > infra/plane/common.py
# then re-apply the bumped throttle rates and commit.
