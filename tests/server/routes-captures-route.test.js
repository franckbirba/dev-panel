...
d('POST /api/captures/:id/route', () => {
  let app, storage, project, key, captureId;
  let createRouter;
  ...
  beforeAll(async () => {
    ...
    ({ createCapture } = await import('../../src/server/captures.js'));
  }, 60000);
  ...
  beforeEach(async () => {
    await truncateTeam();
    storage = mkdtempSync(join(tmpdir(), 'devpanel-capture-route-'));
  ...
  it('routes a capture and returns member + dev_bot', async () => {
    const r = await supertest(app)
      .post(`/api/captures/${captureId}/route`)
      .set('X-API-Key', key)
      .send({ label: 'com' });
  ...
  it('is idempotent', async () => {
    await supertest(app).post(`/api/captures/${captureId}/route`).set('X-API-Key', key).send({ label: 'com' });
    const r = await supertest(app).post(`/api/captures/${captureId}/route`).set('X-API-Key', key).send({ label: 'com' });
    expect(r.body.already_routed).toBe(true);
  });

  it('409 when label has no member', async () => {
    const r = await supertest(app).post(`/api/captures/${captureId}/route`).set('X-API-Key', key).send({ label: 'nope' });
    expect(r.status).toBe(409);
  });

  it('404 when capture does not exist', async () => {
    const r = await supertest(app).post('/api/captures/nonexistent-uuid-99999/route').set('X-API-Key', key).send({ label: 'com' });
